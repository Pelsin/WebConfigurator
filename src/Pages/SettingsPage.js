...
export default function SettingsPage() {
	...
	const {
		state: { gamepadOptions },
		update,
	} = useStateContext();

	const onSuccess = async (values) => {
		update(WebApi.setGamepadOptions(values));
	};
				...
				<div>
					<Form noValidate onSubmit={handleSubmit}>
						<Section title="Settings">
							...
							<p>
								Note: PS4, PS3 and Nintendo Switch modes do not support setting
								SOCD Cleaning to Off and will default to Neutral SOCD Cleaning
								mode.
							</p>
						</Section>
						<Section title="Hotkey Settings">
							<div className="row">
								<Form.Label className="col">
									{BUTTONS[buttonLabels]["S1"] +
										" + " +
										BUTTONS[buttonLabels]["S2"]}
								</Form.Label>
							</div>
